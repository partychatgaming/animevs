                ...
                try:
                    ...
                    if button_ctx.custom_id == "glory":
                        ...
                        if battle.is_hard_difficulty:
                            battle._boss_tactics = ['DAMAGE_CHECK', 'STAGGER', 'DEATH_BLOW']
    ...
    try:
        if user_data['QUESTS']:
            ...
            if str(mode) == "Dungeon" and completion >= 0:
                ...
                if completion == 0:
                    await crown_utilities.bless(reward, str(player.id))
        ...
        if not available_universes:
            if mode in crown_utilities.DUNGEON_M:
                if p.level < 41 and p.prestige <= 40:
                    ...
                    Conquer Level 40 in the Abyss to Unlock Dungeons!
                    ...
                    """))
                    await ctx.send(embed=abyss_lock_embed)
                    return
                else:
            ...
            try:
                ...
                if button_ctx.custom_id == "start_tales_yes" or button_ctx.custom_id == "start_auto_tales":
                    ...
                    while not game_over_check:
                        ...
                        pretrified_check = tactics_petrified_fear_check(player2_card, player1_card, battle_config)
                                ...
                                if battle_config.is_auto_battle_game_mode:                                    
                                            ...
                                            if selected_move != 6:
                                                if player1_card._monstrosity_active and player1_card.used_resolve:
                                                    if player1_card._double_strike_count < player1_card._monstrosity_value:
                                                        ...
                                                        battle_config.add_to_battle_log(f"(**{crown_utilities.class_emojis['MONSTROSITY']}**) **{player1_card.class_message}** : **{player1_card.name}** Double Strike!\n*{player1_card._monstrosity_value - player1_card._double_strike_count} Left!*")
                                                        #damage_calculation_response = player1_card.damage_cal(selected_move, battle_config, player2_card)
                                                        player1_card.damage_done(battle_config, damage_calculation_response, player2_card)
                                    ...
                                    try:
                                        ...
                                        if button_ctx.custom_id == "q" or button_ctx.custom_id == "Q":
                                            player1_card.health = 0
                                            battle_config.game_over = True
                                            battle_config.add_to_battle_log(f"(**{battle_config.turn_total}**) 💨 **{player1_card.name}** Fled...")
                                            await battle_msg.delete(delay=1)
                                            await asyncio.sleep(1)
                                        ...
                                        if button_ctx.custom_id == "1":
                                            ...
                                            if not player1_card.used_resolve and player1_card.used_focus:
                                                ...
                                                battle_config.add_to_battle_log(f"(**{battle_config.turn_total}**) **{player1_card.name}** cannot resolve")
                                                ...
                                                if button_ctx.custom_id != "6" and button_ctx.custom_id != "7":
                                                    if player2_card._monstrosity_active and player2_card.used_resolve:
                                                        if player2_card._double_strike_count < player2_card._monstrosity_value:
                                                            ...
                                                            battle_config.add_to_battle_log(f"(**{crown_utilities.class_emojis['MONSTROSITY']}**) **{player2_card.class_message}** : **{player2_card.name}** Double Strike!\n*{player2_card._monstrosity_value - player2_card._double_strike_count} Left!*")
                                            ...
                                            if selected_move != 6:
                                                if player3_card._monstrosity_active and player3_card.used_resolve:
                                                    if player3_card._double_strike_count < player3_card._monstrosity_value:
                                                        ...
                                                        battle_config.add_to_battle_log(f"(**{crown_utilities.class_emojis['MONSTROSITY']}**) **{player3_card.class_message}** : **{player3_card.name}** Double Strike!\n*{player3_card._monstrosity_value - player3_card._double_strike_count} Left!*")
                                                        #damage_calculation_response = player3_card.damage_cal(selected_move, battle_config, player2_card)
                                                        player3_card.damage_done(battle_config, damage_calculation_response, player2_card)
                                                        battle_config.next_turn()
                        ...
                        try:
                            ...
                            if battle_config.player1_wins and not battle_config.is_pvp_game_mode:
                                if any((battle_config.is_tales_game_mode, battle_config.is_dungeon_game_mode, battle_config.is_boss_game_mode)):
                                    ...
                                    # questlogger = await quest(user1, player2_card.name, battle_config.mode)
                                    # destinylogger = await destiny(user1, player2_card.name, battle_config.mode, None)
                                    ...
                                    if battle_config.is_co_op_mode and not battle_config.is_duo_mode:
                                        ...
                                        p3_questlogger = await quest(user2, player2_card.name, battle_config.mode)
                                        p3_destinylogger = await destiny(user2, player2_card.name, battle_config.mode, None)
