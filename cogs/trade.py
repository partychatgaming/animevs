...
class Trade(commands.Cog):
    ...
    # def is_player_trading(query):
    #     return db.queryTrade(query)

    # def is_player_unlocked_trading(player):
    #     return player['LEVEL'] >= 15

    async def showTrade(ctx, trade):
...
#         if is_player_trading(m_query) or is_player_trading(b_query):
#             message = f"{ctx.author.mention} check your **Trade Status** close your current trade first!"
#             if is_player_trading(m_query):
...
#         }
    ...
    async def trade(self, ctx: SlashContext, mode: str, player = None):
        ...
        if player == None:
            ...
            if trade_check:
                showTrade(trade_check)
            elif trade_check2:
                showTrade(trade_check2)
            else:
            ...
            try:
                ...
                trade_query={'MERCHANT' : str(merchant['NAME']),
                             'BUYER' : str(buyer['NAME']),
                             'MDID': str(ctx.author.id),
                             'BDID' : str(player.id),
                             'OPEN' : True
