...
class Lookup(Extension):
    ...
    async def player(self, ctx, player = None):
        ...
        try:
            ...
            if player_class:
                ...
                ❤️‍🔥 | **Rebirth**: {player_class.rebirth}
                {player_class.prestige_icon} | {prestige_message}
                
                🎴 | **Card:** {player_class.equipped_card}
                🎗️ | **Title:** {player_class.equipped_title}
                🦾 | **Arm:** {player_class.equipped_arm}
                🧬 | **Summon:** {player_class.equipped_summon}
                ...
                ⚙️ | **Battle History:** {str(player_class.battle_history)} messages
    ...
    async def guild(self, ctx, guild = None, player = None):
        ...
        try:
            ...
            if guild_data:
                embed_list = await guild_data.send_guild_info()
                paginator = CustomPaginator.create_from_embeds(self.bot, *embed_list, custom_buttons=guild_data.get_user_buttons(user["DID"]), paginator_type="Guild Buff")
                paginator.show_select_menu = True
                await paginator.send(ctx)
