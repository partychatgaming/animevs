...
class Guild(Extension):
    ...
    async def on_ready(self):
        # print('Assocaition Cog is ready!')
        loggy.info('Association Cog is ready')
                ...
                if guildsearch:
                    ...
                    if sworn_profile['GUILD'] != 'PCG' and guildsearch['SHIELD'] != sworn_profile['DISNAME']:
                        ...
                        if founder_profile['TEAM'] == 'PCG' or sworn_profile['TEAM'] == 'PCG':
                            ...
                            guild_buttons_action_row = ActionRow(*guild_buttons)
                            ...
                            try:
                                button_ctx  = await self.bot.wait_for_component(components=[guild_buttons_action_row], check=check)
                                ...
                                if button_ctx.custom_id == "Yes":
                                    await self.bot.DM(ctx, owner, f"{ctx.author.mention}" + f" would like to join the Association {guild_name}" + f" React in server to join their Association" )
                                    ...
                                    try:
                                        ...
                                        if button_ctx.custom_id == "Yes":
                                            try:
                                                ...
                                                🎴 | **Card: **{sworn_profile['CARD']}
                                                🎗️ | **Title: **{sworn_profile['TITLE']}
                                                🦾 | **Arm: **{sworn_profile['ARM']}
                                                
                                                :ninja: | **Swords: ** 2
                                                **{founder_team['TEAM_NAME']}**
                                                **{sworn_team['TEAM_NAME']}**
                                                💵 | **Guild Split: **{hall_split} 
            ...
            if founder_profile['GUILD'] == 'PCG':
                ...
                try:
                    ...
                    if button_ctx.custom_id == "no":
                        ...
                        return
                    if button_ctx.custom_id == "yes":
                        await self.bot.DM(ctx, owner, f"{ctx.author.mention}" + f" would like to ally with your team!" + f" React in server to join their Association" )
                        trade_buttons = [
                            Button(
                                style=ButtonStyle.GREEN,
                                label="Form Alliance",
        ...
        try:
            ...
            if guild:
                ...
                try:
                    ...
                    if button_ctx.custom_id == "yes":
                        try: 
                            await self.bot.DM(ctx, blade, f"{ctx.author.mention}" + f" would like you to serve as the Association Shield!" + f" React in server to protect the Association" )
                            trade_buttons = [
                                Button(
                                    style=ButtonStyle.GREEN,
                                    label="Serve",
