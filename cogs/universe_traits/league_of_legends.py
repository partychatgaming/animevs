...
def pentakill(player_card, battle_config, opponent_card):
    if player_card.universe == "League Of Legends":
        ...
        battle_config.add_to_battle_log(f"(**{battle_config.turn_total}**) **{player_card.name}** 🩸 Resolved: Pentakill! Dealing {(150 * (player_card.focus_count + opponent_card.focus_count))} damage.")
        return True
    else:
        return False
...
def turret_shot(player_card, battle_config, opponent_card):
    if player_card.universe == "League Of Legends":                
        ...
        battle_config.add_to_battle_log(f"(**🌀**) 🩸 Turret Shot hits **{opponent_card.name}** for **{60 + battle_config.turn_total}** Damage 💥")
