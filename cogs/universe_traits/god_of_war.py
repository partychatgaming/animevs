...
def acension(player_card, battle_config, player_title):
    if player_card.universe == "God Of War":  # God Of War Trait
        ...
        if player_card._gow_resolve:
            ...
            battle_config.add_to_battle_log(f"({battle_config.turn_total}) ♾️ {player_card.name}'s ⚡ascension {title_message}")
        if not player_card._gow_resolve:
            ...
            battle_config.add_to_battle_log(f"({battle_config.turn_total}) ♾️ {player_card.name} refilled their health with an orb")
