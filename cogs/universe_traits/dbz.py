...
def handle_card(card, other_card, battle_config, dmg):
    if card.health <= 0 and card._final_stand and card.universe == "Dragon Ball Z":
        if card.barrier_active and dmg['ELEMENT'] != "PSYCHIC" and not dmg['SUMMON_USED']:
            ...
            battle_config.add_to_battle_log(f"(**{battle_config.turn_total}**) **{card.name}** **{card.name}** disengaged their barrier to engage with an attack")
        battle_config.add_to_battle_log(f"(**{battle_config.turn_total}**) **{card.name}** 🩸 Transformation: Last Stand!!!")
...
def saiyan_spirit(player_card, battle_config, opponent_card):
    if player_card.universe == "Dragon Ball Z":
        ...
        battle_config.add_to_battle_log(f"(**🌀**) 🩸 Saiyan Spirit... You heal for **{opponent_card.stamina + battle_config.turn_total}** ❤️")
