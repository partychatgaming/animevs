...
def cursed_energy(player_card, current_hit_roll, battle_config):
    ...
    if player_card.universe == "Jujutsu Kaisen":
        ...
        if not player_card.jujutsu_kaisen_focus_crit_used:
            ...
            battle_config.add_to_battle_log(f"({battle_config.turn_total}) {player_card.name} is preparing their cursed energy for a critical hit")
...
def cursed_energy_reset(player_card, battle_config):
    if player_card.universe == "Jujutsu Kaisen":
        ...
        battle_config.add_to_battle_log(f"({battle_config.turn_total}) {player_card.name} has reset their cursed energy")
        return
