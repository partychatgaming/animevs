...
def increase_power(player_card, battle_config, opponent_card):
    if opponent_card.universe == "7ds":
        ...
        if opponent_card.usedsummon == False:
            ...
            battle_config.add_to_battle_log(f"({battle_config.turn_total}) 🩸 increased power! {opponent_card.name} increased their stamina 🌀 and Power [ +🗡️ {attack_calculation} | +🛡️{defense_calculation}]")
        else:
            ...
            opponent_card.attack = opponent_card.attack + attack_calculation
            opponent_card.defense = opponent_card.defense + defense_calculation
            battle_config.add_to_battle_log(f"({battle_config.turn_total}) 🩸 power of friendship 🧬 {opponent_card.name} summon rested, {opponent_card.name} increased their stamina 🌀 and Power [ +🗡️ {attack_calculation} | +🛡️{defense_calculation}]")
