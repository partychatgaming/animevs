...
def shinigami_eyes(player_card, battle_config):
    if player_card.universe == "Death Note":
        value = 10
        battle_config.add_to_battle_log(f"({battle_config.turn_total}) {player_card.name} used shinigami eyes to sacrifice {round((.50 * player_card.max_health))} max health to increase turn count by {value + player_card.tier}")
