...
from interactions import Client, ActionRow, Button, ButtonStyle, Intents, listen, slash_command, InteractionContext, SlashCommandOption, OptionType, slash_default_member_permission, SlashCommandChoice, context_menu, CommandType, Permissions, cooldown, Buckets, Embed, Extension, slash_option, AutocompleteContext
...
class GameModes(Extension):
    ...
    def get_ratelimit(self, message: interactions.Message) -> typing.Optional[int]:
        """Returns the ratelimit left"""
        bucket = self._cd.get_bucket(message)
        return bucket.update_rate_limit()

    def get_lvl_ratelimit(self, message: interactions.Message) -> typing.Optional[int]:
    ...
    async def on_message(self, message):
        try:
            if message.author == self.bot.bot.user:
                return #
            ...
            if level_ratelimit is None:
                try:
                    ...
                    if player_that_leveled:
                        ...
                        mode = "Tales"
                        u = await self.bot.bot.fetch_user(str(message.author.id))
